...
let suite =
  [
    (let integers = "(def x 1)"
     ...
     ( integers,
       `Quick,
       simple_emmy_test ~expected_context integers expected_result ));
    (let booleans = "(def x true)"
     and expected_result = "Unit"
     and expected_context = "x = true\n" in
     ( booleans,
       `Quick,
       simple_emmy_test ~expected_context booleans expected_result ));
    (let equality = "(= 1 1)" and expected_result = "true" in
     (equality, `Quick, simple_emmy_test equality expected_result));
    (let program = "(def id (fn x -> x))" and expected_result = "Unit" in
     (program, `Quick, simple_emmy_test program expected_result));
    ...
    (let scoping =
     ...
     (scoping, `Quick, simple_emmy_test scoping expected_result));
    (let scoping =
    ...
    (let multi_arity = "(def multi_arity (fn x y z -> z))"
     ...
     (multi_arity, `Quick, simple_emmy_test multi_arity expected_result));
    (let multi_arity =
    ...
    (let multiplication =
     ...
     ( multiplication,
       `Quick,
       simple_emmy_test ~expected_context multiplication expected_result ));
    (let division = "(def divide (fn x -> (/ x 2))) \n      (divide 2)"
     and expected_result = "1"
     and expected_context = "divide = (function x -> (/ Identifier x 2))\n" in
     ( division,
       `Quick,
       simple_emmy_test ~expected_context division expected_result ));
    (let cond_case = "(cond (= 0 1) 1 (= 2 2) 2 3)" and expected_result = "2" in
     (cond_case, `Quick, simple_emmy_test cond_case expected_result));
    (let and_fn =
       "(def and (fn pred1 pred2 -> (cond (= pred1 false) false (= pred1 \
        pred2) true false)))\n\
       \       (and (= 1 0) (= 2 2))"
     and expected_result = "false" in
     (and_fn, `Quick, simple_emmy_test and_fn expected_result));
    (let and_fn_2 =
       "(def and (fn pred1 pred2 -> (cond (= pred1 false) false (= pred1 \
        pred2) true false)))\n\
       \       (and (= 1 1) (= 2 2))"
     and expected_result = "true" in
     (and_fn_2, `Quick, simple_emmy_test and_fn_2 expected_result));
    (let not_fn = "(def not (fn x -> (cond x false true)))\n       (not 1)"
     and expected_result = "false" in
     (not_fn, `Quick, simple_emmy_test not_fn expected_result));
    (let not_fn_2 =
       "(def not \n\
       \        (fn x -> \n\
       \          (cond \n\
       ...
       \       (not false)"
     and expected_result = "true" in
     (not_fn_2, `Quick, simple_emmy_test not_fn_2 expected_result));
    (let or_fn =
       "(def and \n\
       \        (fn pred1 pred2 -> \n\
       \          (cond \n\
       ...
       \       (or (= 1 1) (= 1 0))"
     and expected_result = "true" in
     (or_fn, `Quick, simple_emmy_test or_fn expected_result));
    (let or_fn_2 =
       "(def and (fn pred1 pred2 -> (cond (= pred1 false) false (= pred1 \
        pred2) true false)))\n\
       \       (def or (fn pred1 pred2 -> (cond (and pred1 pred2) true (= \
       ...
       \       (or (= 0 1) (= 1 1))"
     and expected_result = "true" in
     (or_fn_2, `Quick, simple_emmy_test or_fn_2 expected_result));
    (let or_fn_3 =
       "(def and (fn pred1 pred2 -> (cond (= pred1 false) false (= pred1 \
        pred2) true false)))\n\
       \       (def or (fn pred1 pred2 -> (cond (and pred1 pred2) true (= \
       ...
       \       (or (= 1 1) (= 1 1))"
     and expected_result = "true" in
     (or_fn_3, `Quick, simple_emmy_test or_fn_3 expected_result));
    (let or_fn_4 =
       "(def and \n\
       \        (fn pred1 pred2 -> \n\
       \          (cond \n\
       ...
       \       (or (= 0 1) (= 0 1))"
     and expected_result = "false" in
     (or_fn_4, `Quick, simple_emmy_test or_fn_4 expected_result));
    (let recurse =
       "(def fib (fn x -> (cond (< x 3) 1 (+ (fib (- x 1)) (fib (- x 2))))))\n\
       \      (fib 10)"
     and expected_result = "55" in
     (recurse, `Quick, simple_emmy_test recurse expected_result));
    (let less_than = "(< 5 10)" and expected_result = "true" in
     (less_than, `Quick, simple_emmy_test less_than expected_result));
    (let less_than_or_eq =
       "(def and \n\
       \        (fn pred1 pred2 -> \n\
       \          (cond \n\
       ...
       \       (<= 1 1)"
     and expected_result = "true" in
     (less_than_or_eq, `Quick, simple_emmy_test less_than_or_eq expected_result));
    (let collatz =
       "(def collatz\n\
       \        (fn x count -> \n\
       \          (cond\n\
     ...
     (collatz, `Quick, simple_emmy_test collatz expected_result));
    (let let_bindings =
       "(def l        \n\
       \            (let [x 4]\n\
       \                  x))\n\
     ...
     (let_bindings, `Quick, simple_emmy_test let_bindings expected_result));
    (let let_bindings_in_fn =
     ...
     ( let_bindings_in_fn,
       `Quick,
       simple_emmy_test let_bindings_in_fn expected_result ));
    (let nested_lets =
       "(def l \n\
       \          (let [x 4] \n\
       \            (let [y (+ x 1)]\n\
     ...
     (nested_lets, `Quick, simple_emmy_test nested_lets expected_result));
    (let fns_in_let =
       "(def f\n\
       \          (let [x (fn y -> (+ y 1))] \n\
       \            (x 2)))\n\n\
       ...
       \        f"
     and expected_result = "3" in
     (fns_in_let, `Quick, simple_emmy_test fns_in_let expected_result));
    (let lists_test = "(def f [1 2 3]) f" and expected_result = "[1 2 3]" in
     (lists_test, `Quick, simple_emmy_test lists_test expected_result));
