...
let rec beta_reduce param_id arg expr =
  ...
  | _ -> expr
and alpha_reduce bindings replace expr = 
  ...
  | Parameter id -> 
      ...
      | None -> expr)
  | Def (_, expr) -> alpha_reduce bindings replace expr
  | Fn (params, expr) -> 
    ...
    Fn (new_params, alpha_reduce new_bindings new_replace expr)
  | _ -> expr
...
let rec step expr ctx =  
  ...
  | Fn (params, expr) -> let step_ctx = (List.hd params, Parameter (List.hd params)) :: ctx in (alpha_reduce [] [] (Fn (params, fst (step expr step_ctx))), ctx)
